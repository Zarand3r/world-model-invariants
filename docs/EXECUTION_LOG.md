...
## 2026-08-30 --- Completing the plan-versus-outcome record, and closing out the autonomous queue

The roadmap's execution-status table describes itself as "generated by hand from the run records and
`docs/EXECUTION_LOG.md`", and exists so that what was predicted stays legible next to what happened.
**F8, F9, F10 and F11 had no rows in it** --- four experiments recorded in the log but absent from the
table meant to summarise exactly this. Added, with the two that the retraction voided marked as void
rather than quietly dropped:

- **F8** --- uninformative (flat sweep; the post-hoc contrast gate declared as post-hoc).
- **F9** --- **void**: its `D_scheme` applied Verlet's update to a semi-implicit-convention velocity.
  The floor-control *design* was sound and was reused in F11; the number is not.
- **F10** --- **void**: right design, wrong axis. Kept visible because P1 alone would have read as a
  clean positive at `p < 1e-10` and been meaningless.
- **F11** --- not readable, with the gate that stopped the week's most convincing false positive.

### State of the autonomous queue

Everything reachable without a decision from Richard is now done:

| | status |
|---|---|
| number guards | 37 presence-only -> **0**; all 70 anchored, 26/26 mutations caught |
| provenance | every artefact that reads files records what it read; 70 pre-M29 left honestly unstamped |
| artifact manifest | **50/50** hashes verified against local files, newly repeatable |
| arXiv archive | extracts clean and compiles standalone; **16 pages** as claimed |
| `RESULTS.md` | regenerates byte-identically |
| handoff, `CLAIMS.md`, roadmap | corrected, signposted, and their self-claims audited |
| integrator axis | exhausted --- both instruments closed, and the axis itself withdrawn |
| reframe | drafted with every number recomputed from the run records |

The two live options both need Richard: **spend training compute** on a mixed-timestep or
`dt`-conditioned model to settle the attribution, or **rewrite the manuscript** around the surviving
evidence per `docs/REFRAME_PROPOSAL.md`. I am not going to manufacture further audits to fill the
queue --- at this point that would be motion rather than progress, and the honest report is that the

...
# Execution status  *(maintained; last updated 2026-08-30)*
...
| **F1** balance laws | **DONE -- NEGATIVE, n=3** | Gate 0 passed (relation closes at 0.017 with shadow + midpoint power; the **textbook** balance law explains almost nothing, residual 0.90). Three action-conditioned seeds trained and accepted, action-use 0.740--0.761. The bespoke balance **extractor** was abandoned after three repairs, each of which fixed a real defect without answering the question; a fourth would have been ladder-climbing. Answered instead with the **paper's own validated** `fit_hamiltonian_pair` (amendment 5): it recovers an energy-like scalar on every actuated model, `rho(C, E)` **0.72 / 0.88 / 0.86**, whose change under the transition is essentially uncorrelated with the action's power -- Spearman **0.041 / 0.076 / 0.065** against a registered bar of 0.3, explaining **0.31%** of the variance in `dC`, with observed change 4.5--5.4x larger than predicted. **The model represents the quantity and does not implement the relation it obeys** -- the paper's dissociation extended into the actuated setting. Also explains F5: no balance structure exists for a correction to enforce. |
...
| **F6** timestep scaling | **DONE -- POSITIVE, n=3 x 4 timesteps** | The strongest positive result in the project. A model trained only on pixels recovers a conserved quantity whose correction coefficient tracks `(dt/2) mg(l/2)` across a **4x** range of simulator timestep. Argmin exactly at the predicted `r=1` on **10 of 12** models (P2 4/4); wrong-sign control beaten **12/12** (P4). Slope through the origin **2.484 +/- 0.058** against a parameter-free **2.500**. `rho_obs` at textbook energy grows as `dt^1.41` while the shadow stays at the model's floor, so separation grows `1.03x -> 13.5x`. **P3 FAILS as registered** (intercept CI excludes zero, driven by a real ~12% low bias at the lowest timestep, where the curve is flat to 2%). **The model learns its simulator's numerical scheme, not the physics it approximates.** |
| **F3** constraint residuals | **DONE -- NEGATIVE, gate fired** | A1 fails 3/3: the recovered `G` correlates with the true rod-length residual at `0.004`--`0.032`. Two measured causes: minimising `E[G^2]` ranks the true constraint **1526th of 1819**, and the constraint is barely encoded (held-out `rho` 0.19, residual 4x its own variation). **A quantity that never varies carries no information**, so it lives in decoder weights. Stage B NOT RUN per the gate; walker limb lengths do not vary either. |
| **F5** control return | **DONE -- NEGATIVE, n=3** | Gate 0 passes (+1.65, CI [+0.50, +2.79] over random actions), but no correction arm beats none: largest effect `0.0014` against a return SD of `0.575`, **0.24%**. All four predictions failed. Reason measured: the correction shifts the plan-ranking signal by **0.3%**, because it acts on accumulated drift and a re-planning controller never accumulates any. Survives a 3x horizon; planning quality itself degrades with horizon. |
...
| **F7** second integrator | **INVALID DESIGN -- withdrawn** | Gate 0 first: at a *fixed* `dt`, the schemes want different coefficients (semi-implicit `0.125`, velocity Verlet `0.000`, explicit Euler diverges), so `c*` is a scheme fingerprint and not merely a `dt` fingerprint. Then the model-side test: 3 seeds trained on **velocity Verlet at `dt = 0.05`**, identical to F6's arm in every other respect, put their argmin at **`r = 0` on 3/3** against the semi-implicit models' **`r = 1` on 3/3** -- a mirror image at the same timestep. Each model conserves its *own* scheme's invariant to within `1.11x` of the other's, and textbook energy is conserved **5.15x better** by the Verlet models. P1 and P2 pass; **P3 partial** (pixel std inside F6's range on 2/3, seed 4 outside by 0.0002). **RETRACTED.** A registered cross-evaluation control (amendment 2) shows the sweep's argmin is a pure function of the **evaluation dataset**, not the checkpoint: 0/3 and 0/3 on both off-diagonals, with argmin `+1.0` in all six semi-implicit-eval cells and `0.0` in all six Verlet-eval cells. The measurement asks whether a target is conserved along the given trajectories, not whether the model learned a scheme. **F6 and E19 share this measurement.** A cross-*timestep* control on F6 (amendment 3) is **INCONCLUSIVE**, not exculpatory: off-diagonal sweeps are degenerate (0/6 follow the data, 5/6 pinned at a grid edge) because changing `dt` puts the crossed model out of distribution, whereas F7's same-`dt` scheme change did not. F6 and E19 are therefore **unresolved**. F8 (`docs/F8_PREREG.md`) then tried the imagined-rollout measurement, which removes the evaluation trajectory entirely, and it has **no power**: sweep contrast 1.12x-1.45x against ~5.7x for the one-step measure, and within a single model the argmin spans the **full grid** (spread 4.00) across horizons 2-100. Only `T=10` passes a power gate and there the families do not separate. **Then F10 settled it.**  A forced choice with an exact 50% null and a signed control (`docs/F10_PREREG.md`) shows the preference is a function of the **evaluation data alone**: Verlet-trained models prefer semi-implicit's next state (0.781/0.655/0.729) just as strongly as semi-implicit-trained models do (0.682/0.781/0.745), and both families reverse together on Verlet data. P1 passed 3/3 and P2, the control, failed -- so **that conclusion is itself RETRACTED (2026-08-30)**: semi-implicit Euler and velocity Verlet reduce to the *identical* three-term position recurrence `th_{t+1} = 2 th_t - th_{t-1} + a(th_t) dt^2` (residual 1e-15 on both datasets), differing only in whether `thetadot` is labelled by the backward or central difference -- a variable the pixels never show. F7/F7b/F10 therefore compared two arms that are indistinguishable in the observations, and F6/E19 revert to **UNRESOLVED**, not withdrawn. `dt` *is* observable in the position recurrence, so F6's question stays well posed. **F11** then tested it directly with a forced choice and found it **NOT READABLE**: a model trained at one timestep is ~90x worse on another's data (one-step error 0.007 -> 0.6 rad), giving a prediction error 9x the separation between candidates. A pre-committed gate withheld what would otherwise have read as 0.844/0.852/0.849, p<1e-100. **F6's coefficient claim is therefore not testable by cross-evaluation with these assets** -- a limitation to state, not a falsification. Separating model from data needs a model in distribution on more than one timestep. F4b remains the valid model-vs-data separation, and supports the *level* claim only. Earlier framing kept for the record: **both obvious instruments were closed** -- the one-step statistic is confounded, the imagined-rollout statistic is noise -- so the headline model-side claim currently has no valid measurement. Not shown false; shown unsubstantiable as written. The interventions (E1/E4/E9/E12c) are **immune by construction**: they measure the model's own imagined rollout against a matched null. A valid model-side test would measure conservation along the model's own imagined rollout. Nothing integrated into the paper. |

| **F7b** sign-flip scheme | **INVALID DESIGN -- withdrawn** | Ground truth verified before training: reversing gymnasium's two update lines stays symplectic and puts the optimum at exactly `c = -0.125`, mirroring the forward scheme's `+0.125` with the same separation to one part in a thousand. 3 seeds trained. **Deliberately not analysed** -- its registered analysis uses the measurement falsified by F7 amendment 2, so the number would have no valid interpretation. Checkpoints retained for whenever a valid model-side measurement exists. |

| **F8** imagined rollout | **UNINFORMATIVE** | Removed the evaluation trajectory entirely -- data supplies only `h_0`, every later state from `m.transition`. Rollouts stayed healthy (imagined spread 1.02--1.06x the real spread) but the sweep is **flat**: contrast 1.12x--1.45x across the whole `r` grid against ~5.7x for the one-step measure between `r=0` and `r=1` alone. The script printed "the families do NOT separate: F6/E19 fail"; that was **wrong** -- a flat curve is a statement about the instrument. A contrast gate was added **post-hoc and declared as such**. Horizon sweep 2--100 found one readable horizon (`T=10`) where the families do not separate, and within a *single* model the argmin spans the **full grid** (spread 4.00) across horizons. Noise at every horizon tested. |
| **F9** resolution check | **VOID** (result superseded) | Asked whether the scheme gap is large enough for these models to represent, and measured `D_model/D_scheme` = 0.46 / 0.52 / 0.45 with the decoder floor at 0.34--0.39. Read at the time as "marginal, so the claim is not dead in principle". **Voided by the 2026-08-30 retraction**: `D_scheme` applied velocity Verlet's update formula to a semi-implicit-convention velocity, a category error. The floor-control design was sound and was reused; the number is not. |
| **F10** forced choice (scheme) | **VOID** (invalid axis) | Two-alternative forced choice, exact 50% null, signed control. P1 passed 3/3; **P2, the control, failed** -- Verlet-trained models preferred semi-implicit's next state just as strongly (0.781/0.655/0.729 against 0.682/0.781/0.745), and both families reversed together on Verlet data. Read at the time as falsifying F6/E19. **Voided the same day**: the two schemes are the *same map on positions*, so P2 failing is expected by construction. The design was right and the axis was not. Worth keeping: P1 alone would have read as a clean positive (3/3, `p<1e-10`) and been meaningless. |
| **F11** forced choice (timestep) | **NOT READABLE** | The same test on the axis that *is* observable (`dt` enters the position recurrence through `a(th) dt^2`). G2 fires on every crossed cell: one-step error rises from `~0.007` rad on a model's own data to `~0.6` rad on another timestep's -- a factor of 90, and **9x** the separation the test needs. **The pre-committed gate stopped the week's most convincing false positive**: without it the `dt=0.08` models read as **0.844/0.852/0.849, 3/3, `p<1e-100`**, pure artefact -- the mirrored cells sit at chance (0.512/0.502/0.511), which a real effect would not do. **F6's coefficient claim is therefore not testable by cross-evaluation with these assets**; separating model from data needs a model in distribution on more than one timestep. |

...
why the model side resists measurement here: $\rho_{\mathrm{obs}}$ is computed along evaluation
trajectories, a registered cross-evaluation control shows the argmin follows the evaluation set in
$6/6$ cells and the checkpoint in $0/6$, and the alternatives are a rollout statistic with no power
(contrast $1.12$--$1.45\times$) and a cross-timestep forced choice that is unreadable because a model
evaluated off its training timestep is $\sim\!90\times$ worse in one-step error. Deciding it needs a
model in distribution at more than one timestep. Until then the claim is unsubstantiated rather than
refuted, and the paper's other results are constructed so as not to depend on it.

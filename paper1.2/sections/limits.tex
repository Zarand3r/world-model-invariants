...
\paragraph{The model does not carry the integrator, and we looked.}
Section~\ref{sec:mechanism} reports the negative directly rather than leaving it as an open question.
The explanation of the probe's failure is a property of the training data and survives; the stronger
claim that the network internalised the integrator's coefficient does not. We note that the
experiment which settled it, conditioning the model on its own timestep, is also the one our earlier
instruments could not perform, and that two registered predictors passed on it before we replaced
them with one sensitive to magnitude.
